...
import {
  available_items_library,
  getItemTemplateBy,
} from "../../components/form_builder_items";
...
export default function Home() {
  ...
  const { updateAppName, updateConnectionConfig, applicationsCtx } =
    useApplicationsCtx();
            ...
            targetTable?.fields?.map(async (field) => {
              ...
              if (linkedFields.includes(field.id)) {
                ...
                const targetLink = links
                  ...
                  .endpoints.find(
                    (end_point) => end_point.id !== targetTable.id
                  );
                ...
                return getItemTemplateBy(
                  ...
                  mapDataTypeToElement(field.type),
                  {
                    ...
                    options: tableRows?.map((r, ix) => ({
                      text: r[
                        formBuilderCtx?.optionsConfig[field.id]?.labelField ??
                          null
                      ],
                      value:
                        r[
                          formBuilderCtx?.optionsConfig[field.id]?.valueField ??
                            "_"
                        ],
                      key: ix,
                    })),
  ...
  async function toggleFormPreview() {
    if (formPreview.status === true) {
      ...
      const formPreviewData = await fetch(
        `/api/forms/formdata?formId=${formId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      setFormPreview({
        status: true,
        preview_data: formPreviewData?.task_data ?? [],
      });
      return;
    }
  }
  ...
  useEffect(() => {
    ...
    const existingElements = [...store.state.data].map((elm, ix) => {
      ...
      if (targetElement) {
        const generatedElement = getItemTemplateBy(
          "element",
                    ...
                    return (
                      <Menu.Item key={i}>
                        ...
                        <span>
                          ...
                          >
                            {field_?.isLinked?.status ? <IconLink /> : <></>}
                          </span>
